...
def main():
    ...
    IMG_DIR  = r"D:/Diploma/Plates/Processed_Grayscale_Images"
    ...
    # Трансформации данных - InceptionV3 и Xception ожидают размер 299x299
    train_trans = transforms.Compose([
        transforms.Grayscale(num_output_channels=3),          # Превращаем 1 канал в 3
        transforms.Resize((320, 320)),                        # Увеличенный размер для кропа
        transforms.RandomRotation(30),
        transforms.RandomPerspective(distortion_scale=0.4, p=0.5),
        transforms.RandomHorizontalFlip(0.5),
        transforms.RandomVerticalFlip(0.2),
        transforms.RandomResizedCrop(299, scale=(0.5, 1.0)),  # 299x299 для Inception/Xception
        transforms.ToTensor(),
    ...
    val_trans = transforms.Compose([
        ...
        transforms.Resize((299, 299)),                        # 299x299 для Inception/Xception
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406],
                             [0.229, 0.224, 0.225])
    ])
    ...
    # Будем обучать модели InceptionV3 и Xception
    results_dict = {"inceptionv3": [], "xception": []}
    ...
    for train_idx, val_idx in skf.split(all_paths, all_labels):
        ...
        for arch_name in ["inceptionv3", "xception"]:
            ...
            try:
                ...
                for epoch in range(1, EPOCHS + 1):
                    ...
                    for imgs, labels in train_loader:
                        ...
                        if arch_name == "inceptionv3":
                            # InceptionV3 может возвращать aux_logits при обучении
                            out, aux_out = model(imgs)
                            # Вычисляем основную и вспомогательную потери
                            main_loss = criterion(out, labels)
                            aux_loss = criterion(aux_out, labels)
                            # Общая потеря - взвешенная сумма
                    ...
                    with torch.no_grad():
                        for imgs, labels in val_loader:
                            ...
                            if arch_name == "inceptionv3":
                                # В режиме eval InceptionV3 не возвращает aux_logits
                                outputs = model(imgs)
                            else:
                                outputs = model(imgs)
                ...
                print(f"Ошибка при обучении {arch_name}: {str(e)}")
                results_dict[arch_name].append({"acc": 0.0, "f1": 0.0})

        fold_idx += 1

    print("\n=== Средние метрики по k-fold ===")
    for arch_name in results_dict:
        # Фильтруем только успешные фолды (если были ошибки)
        valid_results = [result for result in results_dict[arch_name] if result["acc"] > 0]
        
        if not valid_results:
            print(f"{arch_name}: Ошибка при обучении всех фолдов")
            continue
            
        acc_values = [result["acc"] for result in valid_results]
        f1_values = [result["f1"] for result in valid_results]
        ...
        print(f"{arch_name}: mean_acc={mean_acc:.3f}±{std_acc:.3f}, mean_f1={mean_f1:.3f}±{std_f1:.3f} (K={len(valid_results)})")
        ...
        with open(results_file, 'w', encoding='utf-8') as f_out:
            ...
            f_out.write(f"K-fold: {len(valid_results)}\n")
...
def get_model(arch_name):
    """
    Загружаем модели InceptionV3, Xception
    с предобученными весами на ImageNet.
    """
    import torch.nn as nn
    num_classes = 2

    if arch_name == "inceptionv3":
        # Включаем aux_logits для InceptionV3 (важно!)
        ...
        # Также заменяем вспомогательный классификатор
        aux_in_f = net.AuxLogits.fc.in_features
        net.AuxLogits.fc = nn.Linear(aux_in_f, num_classes)
        return net
    elif arch_name == "xception":
        # Используем timm для Xception
        net = timm.create_model('legacy_xception', pretrained=True)
        in_f = net.fc.in_features
        net.fc = nn.Linear(in_f, num_classes)
        return net
    else:
...
class MyBinaryDataset(Dataset):
    """
    Базовый датасет для загрузки изображений
    """
...
class TransformWrapper(Dataset):
    """
    Обёртка для применения трансформаций
    """
