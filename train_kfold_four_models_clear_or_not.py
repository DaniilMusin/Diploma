...
def main():
    # Путь к CSV (данные о файлах, label 0/1)
    CSV_PATH = r"D:\Diploma\labels_Denis.csv"
    # Папка с изображениями
    IMG_DIR  = r"D:\Diploma\Plates"
    # Папка, где будут лежать чекпоинты (на D:)
    OUT_DIR  = r"D:\Diploma\checkpoints_kfold"
    os.makedirs(OUT_DIR, exist_ok=True)

    # Считываем (image_path, label), пропуская битые
    data_list = []
...
# MODEL_LIST = ["resnet101","efficientnet_b7","convnext_large","densenet121"]
    ...
    for arch_name in MODEL_LIST:
        ...
        std_acc  = np.std(arr)
        print(f"{arch_name}: mean={mean_acc:.3f}, std={std_acc:.3f}  (K={K})")
...
def get_model(arch_name):
    ...
    if arch_name=="resnet101":
        ...
        raise ValueError("Unknown model: " + arch_name)
